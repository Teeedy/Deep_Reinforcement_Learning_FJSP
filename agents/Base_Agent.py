...
class Base_Agent(object):
    ...
    def __init__(self, config):
        ...
        self.environment = config.environment  # 环境对象
        self.environment_title = self.get_environment_title()  # 环境名称
        self.action_types = "DISCRETE" if self.environment.action_space.dtype == np.int64 else "CONTINUOUS"  # 动作类型【离散|连续】
        self.action_size = int(self.get_action_size())  # 动作维度
        self.config.action_size = self.action_size  # 动作维度
        self.lowest_possible_episode_score = self.get_lowest_possible_episode_score()  # 环境最低周期得分
        self.state_size = int(self.get_state_size())  # 状态维度
        self.hyperparameters = config.hyperparameters  # 超参数
        self.average_score_required_to_win = self.get_score_required_to_win()  # 赢得游戏的平均分数
        self.rolling_score_window = self.get_trials()  # 最近多少个周期的平均得分
        # self.max_steps_per_episode = self.environment.spec.max_episode_steps
        self.total_episode_score_so_far = 0  # 初始化周期总得分
        self.game_full_episode_scores = []  # 所有周期的得分记录列表
        self.rolling_results = []  # 最近self.rolling_score_window个周期的平均得分列表
        self.max_rolling_score_seen = float("-inf")  # 最大平均得分
        self.max_episode_score_seen = float("-inf")  # 最大周期得分
        self.episode_number = 0  # 周期数
        self.device = "cuda:0" if config.use_GPU else "cpu"  # 设备选择
        print("选择的设备：", self.device)
        self.visualise_results_boolean = config.visualise_individual_results  # 结果可视化控制
        self.global_step_number = 0
        self.turn_off_exploration = False  # 探索控制
        gym.logger.set_level(40)  # stops it from printing an unnecessary warning 阻止打印不必要的警告
        self.log_game_info()
    ...
    def get_action_size(self):
        ...
        if self.action_types == "DISCRETE":  # 若动作类型为离散动作
    ...
    def run_n_episodes(self, num_episodes=None, show_whether_achieved_goal=True, save_and_print_results=True):
        ...
        start = time.time()
        while self.episode_number < num_episodes:  # 采集num_episodes （450）条完整轨迹的经验
            self.reset_game()  # 重置环境
            self.step()  # 执行一步：采集一条轨迹并优化对应网络
            if save_and_print_results:
                self.save_and_print_result()  # 保存并打印结果
        time_taken = time.time() - start
        if show_whether_achieved_goal:
            self.show_whether_achieved_goal()  # 输出智能体在哪一个周期实现目标
    ...
    def conduct_action(self, action):
        ...
        if self.hyperparameters["clip_rewards"]:  # 裁剪回报
    ...
    def save_and_print_result(self):
        ...
        """
        self.save_result()  # 保存结果
        self.print_rolling_result()  # 打印结果
    ...
    def save_result(self):
        ...
        self.save_max_result_seen()  # 保存观察到的最好周期结果
    ...
    def take_optimisation_step(self, optimizer, network, loss, clipping_norm=None, retain_graph=False):
        ...
        optimizer.step()  # this applies the gradients 更新梯度
